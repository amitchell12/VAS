...
while answer == 0
    ...
    DrawFormattedText(screenPointer, uint8(question), 'center', rect(4)*(scalaPosition - 0.16), TextColour); 
    
    % Drawing the anchors of the scale as text
    if length(anchors) == 2
        % Only left and right anchors
        DrawFormattedText(screenPointer, uint8(anchors{1}), leftTick(1, 1) - textBounds(1, 3)/2,  rect(4)*scalaPosition+80, TextColour,[],[],[],[],[],[]); % Left point
        DrawFormattedText(screenPointer, uint8(anchors{2}), rightTick(1, 1) - textBounds(2, 3)/2,  rect(4)*scalaPosition+80, TextColour,[],[],[],[],[],[]); % Right point
    else 
        % Left, middle and right anchors
        DrawFormattedText(screenPointer, uint8(anchors{1}), leftTick(1, 1) - textBounds(1, 3)/2,  rect(4)*scalaPosition+80, TextColour,[],[],[],[],[],[]); % Left point
        DrawFormattedText(screenPointer, uint8(anchors{2}), 'center',  rect(4)*scalaPosition+80, TextColour,[],[],[],[],[],[]); % Middle point
        DrawFormattedText(screenPointer, uint8(anchors{3}), rightTick(1, 1) - textBounds(2, 3)/2,  rect(4)*scalaPosition+80, TextColour,[],[],[],[],[],[]); % Right point
    end
